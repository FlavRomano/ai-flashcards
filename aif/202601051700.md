...
### What's the core loop of Genetic Algorithm?
?
1. Selection
2. Crossover
3. Mutation

#### How selection phase works?
?
- From the population
	- pick parents based on a fitness function $$P:p_1,p_2,\dots,p_n$$

##### Which type of selection is used?
?
- Comma selection
	- the $\mu$ best kids will replace the parents

#### How crossover phase works?
?
...
##### What's the problem of direct encoding?
?
...
#### For which encoding the search space is narrow?
?
- Indirect encoding
...
#### Describe search space of direct encoding
?
...
#### What's complexification?
?
Is how the network evolvement mechanism
...
##### Does NEAT use direct encoding?
?
- Yes, NEAT use a **list of genes** to store the network ![[Pasted image 20260106101021.png]] uses a Direct Encoding of the topology.
- Each node  is represented by one gene, same for connections.
	- 1:1 $\implies$ direct encoding

##### What contains each connection gene in NEAT?
?
...
##### What's the innovation number in NEAT?
?
...
##### In NEAT's "Add Node" mutation, how are the weights of the new connections determined?
?
Considering a new node $Y$ inserted on the middle of $(X,Z)$ connection
- in-weight=1
- out-weight=previous weight of $(X,Z)$

###### Why splits connection?
?
- To preserve the flow
	- adds non-linearity (a neuron) in the middle 
	- the network can learn new functions

##### What's NEAT speciation?
?
...
###### How NEAT speciation computes fitness?
?
...
###### In NEAT speciation how competition works?
?
...
###### What problem NEAT speciation tries to mitigate?
?
...
###### What's distance $\delta$?
?
- $\delta$ is a distance score in the topology space
	- based on how many genes don't match (Excess/Disjoint genes) 
	- and how different the weights are

###### How distance $\delta$ is computed?
?
- $\delta$ relates $$\delta = \underbrace{\frac{c_1E}{N} + \frac{c_2D}{N}}_{\text{topology}} + \overbrace{c_3 \Delta W}^{weight}$$
	- topology distance, NEAT aligns the two genomes based on their **Innovation Numbers** ![[Pasted image 20260106110233.png]]
		- $E$ excess genes
		...
		- $N$ number of genes in the larger genome
			- this normalizes the distance so that complex networks aren't considered "distant" just because they are big
	- weight distance, for genes share the same innovation number $$\Delta W$$
		- the algorithm calculates **the average difference in their connection weights** 

#### What's HyperNEAT? Why is used?
?
...
- HyperNEAT use indirect encoding via CPPN ![[Pasted image 20260106111247.png]]
	- evolves a CPPN
		- by adding complexity (node, hidden layers...)
	- nodes are placed on a geometric grid (a substrate). Every node has an $(x,y)$ coordinate.
...
#### What's the novelty search intuition?
?
...
##### How can we define the behaviour of an individual?
?
- By numerical encoding
	- e.g: Maze navigation = path of cartesian coordinates $$[(1,2),(2,3),..,(5,5)]$$

##### How novelty search keeps on archive of phenotypes?
?
...
##### How we define novelty?
?
- Novelty is the sparseness $\rho(x)$ in the behavior space 

##### How $\rho(x)$ sparseness is computed?
?
...
##### Which $\rho(x)$ novelty search seeks? High, low
?
...
##### What's a limitation of novelty search?
?
...
#### What problem solves Quality Diversity?
?
...
##### How does MAP-Elites evaluates the offspring?
?
Given the offspring $x$ MAP-Elites
...
##### How does the MAP-Elites competition/replacement mechanism work?
?
Compare performance against the current bin occupant.
- if $x$ is better then elites, then $x$ is the new bin occupant
- else $x$ is discarded

##### How are parents selected in MAP-Elites?
?
- parents are selected **uniformly at random from the occupied bins**
	- each parent is an elite of that niche
- then parents gets mutated and offspring $x$ is created

##### How do we evaluate MapElites work?
?
...
#### How evolvability judge a parent?
?
- how parent's children are diverse between each other

##### How evolvability search works?
?
...
##### Why Evolutionary Strategy could be applied to Evolvability Search to deal with fake offspring evolution?
?
- Evolutionary Search is expensive because of simulation of fake offsprings
	- Instead of fake offsprings
	- we can maintain just a Gaussian distribution as population $$\pi\sim N(\mu, \sigma)$$

##### What function we want to maximize in Evolvability Search ES?
?
We maximize via gradient descent a function expressing evolvability $$F(\vartheta)=\sum_j \mathbb{E}_{z\sim\pi}\left[ (B_j(z) - \mu_j)^2 \right]$$
- $z\sim\pi$ because the offspring is one sample from $\pi$ distribution
- $j$ is the index of the component behaviour
	- e.g: maze-> final solution $(1,2)$ two components
- $B_j(z)$ is the behaviour of genome $z$ on component $j$
- $\mu_j$ average behaviour of the entire distribution on component $j$
- $(B(z) - \mu)^2$ how far the individual $z$ is from average $\mu$
- $\mathbb E[\dots]$ is the variance
	- averaged squared difference for many samples from the cloud

##### Why Evolvability Search ES want max variance?
?
- Maximizing the variance $$\max \;\sum\underbrace{\mathbb E[(B(z) - \mu)^2]}_{variance}$$
	- the function represents the evolvability
	- pushes the population distribution to "expand"
		- more evolvability => more inflated gaussian cloud
	- and to cover as much behavioral space as possible.
-  Low Evolvability
	- The cloud is a tight dot.
	- Variance is 0. The system is "brittle.
- High Evolvability
	- The cloud expands like an explosion
	- variance is high, the system is ready to adapt.