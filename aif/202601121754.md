...
?
Because the search space for a proof can be infinite due to function symbols  e.g `Father(Father(...))`
- If Entailed (True)
- Herbrand guarantees a finite proof exists, so the algorithm will eventually find it and stop.
- If Not Entailed (False): 
- The algorithm may generate infinite ground terms and loop forever, never being able to distinguish between "False" and "True but deeper in the search space."
<!--SR:!2026-01-21,1,210-->
